...
4. See what's next using the **task list** in the issue body
5. Keep the task list and project fields **updated** as the project progresses
...
- `Dataset` issues are created as sub-issues within `Project` issues.

 In GitHub terminology, projects are **parent issues** and datasets are **sub-issues**. This simply means that multiple `Dataset` issues can be grouped within a parent `Project` issue. Both issue types otherwise have the same structure, with the main difference being that there is slightly more information (i.e. a longer task list) included in the body of `Project` issues.

## 🔎 See everything, or just the things you need
Choose between five views which display issues and their metadata differently depending on use case.
...
Feel free to apply new filters if you need to find something specific, however please don't save these changes as it will override the settings for all other users. The exception to this is the Reporting view which is expected to be updated depending on the needs of the reporting manager. 
